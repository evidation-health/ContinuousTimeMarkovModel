...
datadir = '../data/small_random_sample/'
...
#B_start = np.random.rand(K,M)
#np.random.seed(ranSeed+3)
#B0_start = np.random.rand(K,M)
#np.random.seed(ranSeed+4)
#pi_start = np.random.rand(M)
...
#    steps.append(Constant(vars=[pi]))
    #steps.append(Metropolis(vars=[pi], scaling=0.358, tune=False))
    steps.append(NUTS(vars=[Q],scaling=np.ones(M-1,dtype=float)*10.))
    #steps.append(Metropolis(vars=[Q], scaling=0.25, tune=False))
...
#    steps.append(Constant(vars=[S]))
    ...
    #steps.append(Metropolis(vars=[B0], scaling=0.25, tune=False))
#    steps.append(Constant(vars=[B0]))
#    steps.append(Constant(vars=[B]))
    #steps.append(Metropolis(vars=[B], scaling=0.2, tune=False))
...
#    steps.append(Constant(vars=[Z]))
    ...
    #steps.append(Metropolis(vars=[L],scaling=0.04, tune=False, ))
